...
class TestMotorhat(NIOBlockTestCase):
    def test_process_signals(self):
        ...
        with patch(Motorhat.__module__ + '.Adafruit_MotorHAT') as mock_MotorHAT, \
            ...
            self.assertEqual(mock_DCMotor.return_value.getMotor.setSpeed.call_count,len(input_signal))
            for key in input_signal:
                mock_DCMotor.return_value.getMotor.setSpeed.assert_any_call(abs(input_signal[key]))
